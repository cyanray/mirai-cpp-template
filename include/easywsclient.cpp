...
{ // private module-only namespace
	...
	public:
		...
		_RealWebSocket(socket_t sockfd, bool useMask)
			: sockfd(sockfd), readyState(OPEN), interruptIn(0)
			, interruptOut(0), useMask(useMask), isRxBad(false)
		{
#ifdef _WIN32
			INT rc;
			WSADATA wsaData;
		...
		{ // timeout in milliseconds
			...
			while (true)
			{
				...
				else if (ret <= 0)
				{
					...
					closesocket(sockfd);
					readyState = CLOSED;
					throw std::runtime_error(ret < 0 ? "Connection error!" : "Connection closed!");
			...
			while (true)
			{
				...
				else if (ws.N0 == 127)
				{
					...
					if (ws.N & 0x8000000000000000ull)
					{
						...
						close();
						throw std::runtime_error("ERROR: Frame has invalid frame length. Closing.");
						return;
					}
				}
				if (ws.mask)
				...
				else
				{
					...
					throw std::runtime_error("ERROR: Got unexpected WebSocket message.");
	...
	easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin)
	{
		...
		if (url.size() >= 512)
		{
			throw std::runtime_error("ERROR: url size limit exceeded: " + url);
			// fprintf(stderr, "ERROR: url size limit exceeded: %s\n", url.c_str());
			return NULL;
		}
		if (origin.size() >= 200)
		{
			throw std::runtime_error("ERROR: origin size limit exceeded: " + origin);
		...
		{
			throw std::runtime_error("ERROR: Could not parse WebSocket url: " + url);
		...
		if (sockfd == INVALID_SOCKET)
		{
			throw std::runtime_error("Unable to connect to " + std::string(host) + ":" + std::to_string(port));
